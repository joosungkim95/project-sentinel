...
**Bug #6 — Shadow live fills, intentionally deferred.** Pre-fix shadow_stats: 22/23 signals failed on the live side (paper succeeded for all 23, that's how we have a trades table at all). Predictions *should* be routing through Kalshi `observe_only=True` → `_simulate_fill()` → `executed=True`, and 8 of the 23 signals are predictions, so the math says `live_executed >= 9`. But it was 1. The contradiction means *either* observe_only isn't True at runtime, *or* the Kalshi adapter isn't routing through the shadow executor at all, *or* there's a separate failure path. Could not diagnose pre-deploy because the failure-side log line was being dropped by Bug #3. Marked open; will be the first thing to investigate after the deploy lands and INFO logs are flowing.
