...
- [x] Add `real_money_total` field to PortfolioSnapshot — separates ~$100k Alpaca paper from ~$132 real (Coinbase USD top-up + BTC/ETH positions + Kalshi balance). Verified live: paper=$99,999.69, real=$132.39. Discord risk-event alerts now show both numbers. Adapters expose `is_paper` + `real_money_value()` (Alpaca flips on URL, Coinbase/Kalshi explicitly real). Migration `c3d4e5f6a7b8`.
