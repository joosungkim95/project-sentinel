...
class EquityTrendFollowingStrategy(Strategy):
    ...
    def _analyze_symbol(
        ...
        # --- BUY signal (OR-relaxed: EMA is primary, ADX/price boost) ---
        ...
        # Primary: EMA bullish. ADX and price-above are confluence boosts.
        if ema_bullish and (adx_trending or price_above_ema):
            ...
            return [
                Signal(
                    ...
                    rationale=(
                        f"Equity Trend BUY: {' + '.join(triggers)}"
                    ),
    ...
    def _calc_buy_confidence(
        ...
        """
        Calculate buy confidence with OR-relaxed confluence.

        EMA bullish is primary (always present). ADX and price-above
        are confluence boosts.
        """
        # EMA spread (0.25-0.40): wider spread = stronger trend
        if slow_ema > 0:
            spread_pct = abs(fast_ema - slow_ema) / slow_ema * 100
        else:
            spread_pct = 0.0
        ema_score = min(0.25 + spread_pct / 4.0, 0.40)
        ...
        adx_score = 0.0
        if adx_trending:
            adx_score = min(0.10 + (adx - 20.0) / 40.0, 0.25)
